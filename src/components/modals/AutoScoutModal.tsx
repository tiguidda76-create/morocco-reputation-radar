...
export const AutoScoutModal: React.FC<AutoScoutModalProps> = ({
  ...
  const [selectedRegion, setSelectedRegion] = useState<MoroccanRegion>('Marrakech-Safi');
  const [selectedCity, setSelectedCity] = useState('Marrakech (Médina & Guéliz)');
  const [selectedCategory, setSelectedCategory] = useState<VenueCategory>('Riad de Luxe');
  ...
  const handleStartScan = async () => {
    setIsScanning(true);
    setScanProgress(5);
    setScanStatusText('Connexion aux sources de données multi-plateformes...');
    setDiscoveredVenues([]);
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-white font-display">
                  Scout IA : Découverte Automatique de Prospects
                </h3>
              ...
              <p className="text-xs text-slate-400">
                Scannez et qualifiez automatiquement les Riads, Hôtels et Restaurants avec des avis non répondus.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-3">
              ...
              <div className="space-y-2 text-xs">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setSelectedCity(cityOptionsByRegion[reg][0] || 'Marrakech');
                    }}
                    className="w-full p-2 bg-slate-900 border border-slate-700 rounded-xl text-slate-200 focus:outline-none"
                  >
                ...
                <div>
                  <label className="text-slate-400 block text-[11px] mb-1">Ville / Quartier Cible :</label>
                  <select
                    value={selectedCity}
            ...
            <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-3">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div>
                  <label className="text-slate-400 block text-[11px] mb-1">Nombre d'établissements à extraire :</label>
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="pt-1">
                  <button
                    ...
                    disabled={isScanning}
                    className="w-full py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-emerald-950/50 flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {isScanning ? (
                      <>
                      ...
                      <>
                        ...
                        <span>Lancer le Scan Automatique</span>
                      </>
                    )}
                  </button>
                </div>
            ...
            <div className="p-4 bg-slate-950 rounded-2xl border border-emerald-500/40 space-y-2">
              <div className="flex items-center justify-between text-xs font-mono">
                <span className="text-emerald-400 flex items-center gap-1.5">
                  <RefreshCw className="w-3.5 h-3.5 animate-spin" /> {scanStatusText}
                </span>
              ...
              <div className="p-4 rounded-2xl bg-emerald-950/30 border border-emerald-500/40 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-sm font-bold text-white block">
                      {discoveredVenues.length} Établissements Qualifiés Détectés
                    </span>
                    <span className="text-xs text-slate-400">
                      Tous équipés de numéros WhatsApp marocains et d'estimations de pertes chiffrées.
                    </span>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <span className="text-base font-extrabold text-amber-400 font-mono">
                    {totalCalculatedLoss.toLocaleString()} MAD / an
                  </span>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-800/60 text-slate-300 font-mono text-[11px]">
                    {discoveredVenues.map((v) => (
                      <tr key={v.id} className="hover:bg-slate-900/40 transition-colors">
                        ...
                        <td className="py-2.5 px-3 text-slate-400">{v.city}</td>
                        <td className="py-2.5 px-3">
                          <span className="text-amber-400 font-bold">★ {v.overallScore}</span>
                          <span className="text-rose-400 ml-2">({v.unrepliedReviews} sans réponse)</span>
                        </td>
        ...
        <div className="px-6 py-4 bg-slate-950 border-t border-slate-800 flex items-center justify-between">
          ...
          {discoveredVenues.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-emerald-950/50"
            >
