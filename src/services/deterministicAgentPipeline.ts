...
ICE : 1161674000043`,
  ...
  const whatsAppPitchCopy = {
    ...
    frenchMessage: `Bonjour ${venue.contactPerson || 'Direction'},\n\nNous avons audité la réputation de "${venue.name}" à ${venue.city} : ${risk.unrepliedCountTotal} avis sans réponse génèrent une fuite estimée à -${risk.computedAnnualLossMAD.toLocaleString()} MAD/an.\n\nConsultez l'audit complet : ${auditPublicUrl}\n\nDiscutons-en 5 min par WhatsApp au 0632155430.`,
    waDirectLink: `https://wa.me/${moroccanPhone}?text=${encodeURIComponent(`Salam Si/Lalla ${venue.contactPerson || 'Gérant'} 👋, M3ak Hassan Tiguidda men Agence Morocco Radar. Khedemna audit rapide 3la "${venue.name}" f ${venue.city} : l9ina ${risk.unrepliedCountTotal} avis bla jawb (perte: ~${risk.computedAnnualLossMAD.toLocaleString()} MAD/an). Chof l-audit dialk hna : ${auditPublicUrl} | 📞 0632155430`)}`
  };
